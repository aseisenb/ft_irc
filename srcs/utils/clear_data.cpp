...
void	clear_data(t_data &data)
{
	...
	vector<int>::iterator  it_open_fds = data.open_fd.begin();
	vector<int>::iterator  ite_open_fds = data.open_fd.end();
...
void	clear_exit_data(t_data &data, std::string message, int err_code)
{
	clear_data(data);
	if (err_code)
		error(message);
